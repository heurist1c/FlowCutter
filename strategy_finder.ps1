        ...
        foreach ($bat in $batFiles) {
            ...
            if ($rawLines) {
                ...
                if ($parts.Count -gt 0) {
                    ...
                    $proc = $null
                    try {
                        $batWrapper = Join-Path $env:TEMP "flowcutter_scan_winws.bat"
                        $cmdLine = "`"$exe`" $cmd"
            ...
            while ($waited -lt 5000) {
                ...
                if ($scanPid -gt 0) {
                    ...
                    if ($found) { $winwsRunning = $true; break }
                }
            }
            ...
            if (-not $winwsRunning -and $scanPid -gt 0) {
                try {
                    $p = Get-Process -Id $scanPid -EA SilentlyContinue
                    if ($p) { "winws EXIT CODE: $($p.ExitCode)" | Out-File $debugLog -Append -Encoding UTF8 }
                } catch {}
            }
...
$BtnRestart.Add_Click({
    ...
    [void][System.Windows.Threading.Dispatcher]::CurrentDispatcher.BeginInvoke(
        ...
        [System.Windows.Threading.DispatcherOperationCallback]{
            param($o); $frame.Continue = $false; return $null
        }, $null)
    [void][System.Windows.Threading.Dispatcher]::CurrentDispatcher.PushFrame($frame)
...
# --- System Tray ---
$null = Add-Type -MemberDefinition '[DllImport("user32.dll")] public static extern bool DestroyIcon(IntPtr handle);' -Name "IconHelper" -Namespace "Win32" -PassThru
